...
export default {
  ...
  data() {
    ...
    return {  allSteps, allNodes, blobs, allLines, coordinatesOfNodes, countNumbers,
      projectId, pipelineId, destination, source, modalErrorMessage, pipelineName,
      parameter, steptest, paramWithval }
    },
    async created() {
      let here = this;
      let highestNumber = 0;
    ...
    methods: {
      ...
      updateLines: function () {
        ...
        function dragended(d) {
          ...
          here.coordinatesOfNodes.forEach( function(node) {
            if ((d3.event.x + start <= (node.coords[0] + 15) && d3.event.x + start >= (node.coords[0] - 15))
             && (d3.event.y + end <= (node.coords[1] + 15) && d3.event.y + end >= (node.coords[1] - 15)) ) {
