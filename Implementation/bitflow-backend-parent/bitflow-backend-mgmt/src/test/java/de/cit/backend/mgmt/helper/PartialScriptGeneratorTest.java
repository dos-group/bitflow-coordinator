...
public class PartialScriptGeneratorTest extends AbstractPipelineTest{
	...
	private DeploymentInformation[] generateScripts(PipelineDTO pipeline, int size) throws BitflowException {
		PipelineDistributer builder;
		if(size >= 0){
			builder = new PipelineDistributer(size);
		}else{
			builder = new PipelineDistributer();
		}
		PartialScriptGenerator generator = new PartialScriptGenerator();
